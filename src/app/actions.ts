...
export async function sendChart(data: ChartData, user: User) {
  if (data) {
    ...
    const storeUserResp = await fetch(`${process.env.HOST}/api/store-user`, {
      method: 'POST',
    ...
    const storeChartResp = await fetch(`${process.env.HOST}/api/store-chart`, {
      method: 'POST',
      headers: localHeaders,
      body: JSON.stringify({
        userId: newUser.id,
    ...
    // Now send it
    await fetch(`${process.env.HOST}/api/email-chart`, {
      method: 'POST',
      headers: localHeaders,
      body: JSON.stringify({
        chartId: newChart.id,
