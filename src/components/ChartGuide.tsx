...
import GuideToFlowHero from './GuideToFlowHero'
...
async function fetchChartData(chartId: string) {
  const response = await fetch(
    `https://${process.env.HOST}/api/get-chart?id=${chartId}`,
    {
...
export default async function ChartGuide({ chartId }: ChartGuideProps) {
  ...
  return (
    <>
      <GuideToFlowHero firstName={firstName} lastName={lastName} />
      ...
      <div className="bg-white px-6 py-8 lg:px-8">
        <div className="mx-auto max-w-2xl text-base leading-7 text-gray-700">
          <p id="top" className="mt-8 text-xl leading-8">
            ...
            be someone we aren't, and so it can be hard to recognize our true
            nature.
          </p>
          <p className="mt-6">
            Why is that? Well our parents, doing their best, wanted to guide us
            by their own experiences. Unfortunately, what worked for them is
            often different from what we need. We are all different! But our
            parents, they being our only source of love as a child, we would do
            absolutely anything not lose it ... even if that means going against
            our very nature. And this process of "fitting in" repeats itself at
          ...
          <p className="mt-8">
            ...
            tools bring us back to our true nature: that each of these bodies
            are different and how to trust the guidance we receive daily.
          </p>
          <p className="mt-6">
            One final suggestion: treat everything you read here as an
            opportunity to experiment. Try it out, see how it works, and then
            take it or leave it; don't believe anything that anyone tells you,
            even what you read here! Enjoy the process. Let go of what you{' '}
            <em>think</em> you need to do and relax into your body's wisdom.
          </p>
